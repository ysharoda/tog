...
section BoundedModularLattice   
  ...
  structure Hom  {A1 : Type} {A2 : Type} (Bo1 : (BoundedModularLattice A1)) (Bo2 : (BoundedModularLattice A2)) : Type := 
       (hom : (A1 → A2))
  ...
  structure RelInterp  {A1 : Type} {A2 : Type} (Bo1 : (BoundedModularLattice A1)) (Bo2 : (BoundedModularLattice A2)) : Type 1 := 
       (interp : (A1 → (A2 → Type)))
  ...
  inductive BoundedModularLatticeTerm   : Type  
     | timesL : (BoundedModularLatticeTerm → (BoundedModularLatticeTerm → BoundedModularLatticeTerm)) 
     | plusL : (BoundedModularLatticeTerm → (BoundedModularLatticeTerm → BoundedModularLatticeTerm)) 
     | zeroL : BoundedModularLatticeTerm 
     | oneL : BoundedModularLatticeTerm  
      open BoundedModularLatticeTerm 
  
  inductive ClBoundedModularLatticeTerm  (A : Type) : Type  
     | sing : (A → ClBoundedModularLatticeTerm) 
     | timesCl : (ClBoundedModularLatticeTerm → (ClBoundedModularLatticeTerm → ClBoundedModularLatticeTerm)) 
     | plusCl : (ClBoundedModularLatticeTerm → (ClBoundedModularLatticeTerm → ClBoundedModularLatticeTerm)) 
     | zeroCl : ClBoundedModularLatticeTerm 
     | oneCl : ClBoundedModularLatticeTerm  
      open ClBoundedModularLatticeTerm 
  
  inductive OpBoundedModularLatticeTerm  (n : ℕ) : Type  
     | v : ((fin n) → OpBoundedModularLatticeTerm) 
     | timesOL : (OpBoundedModularLatticeTerm → (OpBoundedModularLatticeTerm → OpBoundedModularLatticeTerm)) 
     | plusOL : (OpBoundedModularLatticeTerm → (OpBoundedModularLatticeTerm → OpBoundedModularLatticeTerm)) 
     | zeroOL : OpBoundedModularLatticeTerm 
     | oneOL : OpBoundedModularLatticeTerm  
      open OpBoundedModularLatticeTerm 
  
  inductive OpBoundedModularLatticeTerm2  (n : ℕ) (A : Type) : Type  
     | v2 : ((fin n) → OpBoundedModularLatticeTerm2) 
     | sing2 : (A → OpBoundedModularLatticeTerm2) 
     | timesOL2 : (OpBoundedModularLatticeTerm2 → (OpBoundedModularLatticeTerm2 → OpBoundedModularLatticeTerm2)) 
     | plusOL2 : (OpBoundedModularLatticeTerm2 → (OpBoundedModularLatticeTerm2 → OpBoundedModularLatticeTerm2)) 
     | zeroOL2 : OpBoundedModularLatticeTerm2 
     | oneOL2 : OpBoundedModularLatticeTerm2  
      open OpBoundedModularLatticeTerm2 
  
  def simplifyCl   (A : Type)  : ((ClBoundedModularLatticeTerm A) → (ClBoundedModularLatticeTerm A)) 
  | (plusCl zeroCl x) := x  
  | (plusCl x zeroCl) := x  
  | (timesCl oneCl x) := x  
  | (timesCl x oneCl) := x  
  ...
  | oneCl := oneCl  
  | (sing x1) := (sing x1)  
  def simplifyOpB   (n : ℕ)  : ((OpBoundedModularLatticeTerm n) → (OpBoundedModularLatticeTerm n)) 
  | (plusOL zeroOL x) := x  
  | (plusOL x zeroOL) := x  
  | (timesOL oneOL x) := x  
  | (timesOL x oneOL) := x  
  ...
  | oneOL := oneOL  
  | (v x1) := (v x1)  
  def simplifyOp   (n : ℕ) (A : Type)  : ((OpBoundedModularLatticeTerm2 n A) → (OpBoundedModularLatticeTerm2 n A)) 
  | (plusOL2 zeroOL2 x) := x  
  | (plusOL2 x zeroOL2) := x  
  | (timesOL2 oneOL2 x) := x  
  | (timesOL2 x oneOL2) := x  
  ...
  | (v2 x1) := (v2 x1)  
  | (sing2 x1) := (sing2 x1)  
  def evalB   {A : Type}  : ((BoundedModularLattice A) → (BoundedModularLatticeTerm → A)) 
  | Bo (timesL x1 x2) := ((times Bo) (evalB Bo x1) (evalB Bo x2))  
  | Bo (plusL x1 x2) := ((plus Bo) (evalB Bo x1) (evalB Bo x2))  
  | Bo zeroL := (zero Bo)  
  | Bo oneL := (one Bo)  
  def evalCl   {A : Type}  : ((BoundedModularLattice A) → ((ClBoundedModularLatticeTerm A) → A)) 
  | Bo (sing x1) := x1  
  | Bo (timesCl x1 x2) := ((times Bo) (evalCl Bo x1) (evalCl Bo x2))  
  | Bo (plusCl x1 x2) := ((plus Bo) (evalCl Bo x1) (evalCl Bo x2))  
  | Bo zeroCl := (zero Bo)  
  | Bo oneCl := (one Bo)  
  def evalOpB   {A : Type} (n : ℕ)  : ((BoundedModularLattice A) → ((vector A n) → ((OpBoundedModularLatticeTerm n) → A))) 
  ...
  def evalOp   {A : Type} (n : ℕ)  : ((BoundedModularLattice A) → ((vector A n) → ((OpBoundedModularLatticeTerm2 n A) → A))) 
  ...
  def inductionB   (P : (BoundedModularLatticeTerm → Type))  : ((∀ (x1 x2 : BoundedModularLatticeTerm) , ((P x1) → ((P x2) → (P (timesL x1 x2))))) → ((∀ (x1 x2 : BoundedModularLatticeTerm) , ((P x1) → ((P x2) → (P (plusL x1 x2))))) → ((P zeroL) → ((P oneL) → (∀ (x : BoundedModularLatticeTerm) , (P x)))))) 
  ...
  def inductionCl   (A : Type) (P : ((ClBoundedModularLatticeTerm A) → Type))  : ((∀ (x1 : A) , (P (sing x1))) → ((∀ (x1 x2 : (ClBoundedModularLatticeTerm A)) , ((P x1) → ((P x2) → (P (timesCl x1 x2))))) → ((∀ (x1 x2 : (ClBoundedModularLatticeTerm A)) , ((P x1) → ((P x2) → (P (plusCl x1 x2))))) → ((P zeroCl) → ((P oneCl) → (∀ (x : (ClBoundedModularLatticeTerm A)) , (P x))))))) 
  ...
  def inductionOpB   (n : ℕ) (P : ((OpBoundedModularLatticeTerm n) → Type))  : ((∀ (fin : (fin n)) , (P (v fin))) → ((∀ (x1 x2 : (OpBoundedModularLatticeTerm n)) , ((P x1) → ((P x2) → (P (timesOL x1 x2))))) → ((∀ (x1 x2 : (OpBoundedModularLatticeTerm n)) , ((P x1) → ((P x2) → (P (plusOL x1 x2))))) → ((P zeroOL) → ((P oneOL) → (∀ (x : (OpBoundedModularLatticeTerm n)) , (P x))))))) 
  ...
  def inductionOp   (n : ℕ) (A : Type) (P : ((OpBoundedModularLatticeTerm2 n A) → Type))  : ((∀ (fin : (fin n)) , (P (v2 fin))) → ((∀ (x1 : A) , (P (sing2 x1))) → ((∀ (x1 x2 : (OpBoundedModularLatticeTerm2 n A)) , ((P x1) → ((P x2) → (P (timesOL2 x1 x2))))) → ((∀ (x1 x2 : (OpBoundedModularLatticeTerm2 n A)) , ((P x1) → ((P x2) → (P (plusOL2 x1 x2))))) → ((P zeroOL2) → ((P oneOL2) → (∀ (x : (OpBoundedModularLatticeTerm2 n A)) , (P x)))))))) 
  ...
  def stageB  : (BoundedModularLatticeTerm → (Staged BoundedModularLatticeTerm))
  | (timesL x1 x2) := (stage2 timesL (codeLift2 timesL) (stageB x1) (stageB x2))  
  | (plusL x1 x2) := (stage2 plusL (codeLift2 plusL) (stageB x1) (stageB x2))  
  | zeroL := (Now zeroL)  
  | oneL := (Now oneL)  
  def stageCl   (A : Type)  : ((ClBoundedModularLatticeTerm A) → (Staged (ClBoundedModularLatticeTerm A))) 
  | (sing x1) := (Now (sing x1))  
  | (timesCl x1 x2) := (stage2 timesCl (codeLift2 timesCl) (stageCl x1) (stageCl x2))  
  | (plusCl x1 x2) := (stage2 plusCl (codeLift2 plusCl) (stageCl x1) (stageCl x2))  
  | zeroCl := (Now zeroCl)  
  | oneCl := (Now oneCl)  
  def stageOpB   (n : ℕ)  : ((OpBoundedModularLatticeTerm n) → (Staged (OpBoundedModularLatticeTerm n))) 
  | (v x1) := (const (code (v x1)))  
  | (timesOL x1 x2) := (stage2 timesOL (codeLift2 timesOL) (stageOpB x1) (stageOpB x2))  
  | (plusOL x1 x2) := (stage2 plusOL (codeLift2 plusOL) (stageOpB x1) (stageOpB x2))  
  | zeroOL := (Now zeroOL)  
  | oneOL := (Now oneOL)  
  def stageOp   (n : ℕ) (A : Type)  : ((OpBoundedModularLatticeTerm2 n A) → (Staged (OpBoundedModularLatticeTerm2 n A))) 
