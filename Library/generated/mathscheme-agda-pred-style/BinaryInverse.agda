...
module BinaryInverse   where
  ...
  record IsBinaryInverse  (A : Set) (|> : (A → (A → A))) (<| : (A → (A → A))) : Set where 
     field  
  ...
  record BinaryInverse  (A : Set) : Set where 
     field  
      |> : (A → (A → A)) 
      <| : (A → (A → A)) 
      isBinaryInverse : (IsBinaryInverse A |> <|)  
  ...
  evalOpB n Bi vars (v x1) = (lookup vars x1)  
  evalOpB n Bi vars (|>OL x1 x2) = ((|> Bi) (evalOpB n Bi vars x1) (evalOpB n Bi vars x2))  
  evalOpB n Bi vars (<|OL x1 x2) = ((<| Bi) (evalOpB n Bi vars x1) (evalOpB n Bi vars x2))  
  evalOp :  {A : Set} (n : Nat) →  ((BinaryInverse A) → ((Vec A n) → ((OpBinaryInverseTerm2 n A) → A))) 
  evalOp n Bi vars (v2 x1) = (lookup vars x1)  
  evalOp n Bi vars (sing2 x1) = x1  
  evalOp n Bi vars (|>OL2 x1 x2) = ((|> Bi) (evalOp n Bi vars x1) (evalOp n Bi vars x2))  
  evalOp n Bi vars (<|OL2 x1 x2) = ((<| Bi) (evalOp n Bi vars x1) (evalOp n Bi vars x2))  
