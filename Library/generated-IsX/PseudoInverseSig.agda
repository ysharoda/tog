 ...
 module PseudoInverseSig  where
  ...
  record IsPseudoInverseSig (A  : Set ) (inv  : (A  → A )) (op  : (A  → (A  → A )))  : Set where
    constructor IsPseudoInverseSigC
  ...
  record Hom (A1 A2  : Set ) (Ps1  : (PseudoInverseSig A1 )) (Ps2  : (PseudoInverseSig A2 ))  : Set where
    constructor HomC
    field
      hom : (A1 → A2)
  ...
  record RelInterp (A1 A2  : Set ) (Ps1  : (PseudoInverseSig A1 )) (Ps2  : (PseudoInverseSig A2 ))  : Set₁ where
  ...
  simplifyB : (PseudoInverseSigTerm  → PseudoInverseSigTerm )
  ...
  simplifyOp : ((n  : Nat)  → ((OpPseudoInverseSigTerm n ) → (OpPseudoInverseSigTerm n )))
  ...
  simplifyOpE : ((n  : Nat ) (A  : Set )  → ((OpPseudoInverseSigTerm2 n A ) → (OpPseudoInverseSigTerm2 n A )))
  ...
  evalOp : ({A  : Set } (n  : Nat)  → ((PseudoInverseSig A ) → ((Vec A n ) → ((OpPseudoInverseSigTerm n ) → A ))))
  ...
  evalOpE : ({A  : Set } (n  : Nat )  → ((PseudoInverseSig A ) → ((Vec A n ) → ((OpPseudoInverseSigTerm2 n A ) → A ))))
  ...
  inductionOp : ((n  : Nat) (P  : ((OpPseudoInverseSigTerm n ) → Set ))  → (((fin  : (Fin n ))  → (P (v fin ) )) → (((x1  : (OpPseudoInverseSigTerm n ) )  → ((P x1 ) → (P (invOL x1 ) ))) → (((x1 x2  : (OpPseudoInverseSigTerm n ) )  → ((P x1 ) → ((P x2 ) → (P (opOL x1 x2 ) )))) → ((x  : (OpPseudoInverseSigTerm n ))  → (P x ))))))
  ...
  inductionOpE : ((n  : Nat ) (A  : Set ) (P  : ((OpPseudoInverseSigTerm2 n A ) → Set ))  → (((fin  : (Fin n ))  → (P (v2 fin ) )) → (((x1  : A )  → (P (sing2 x1 ) )) → (((x1  : (OpPseudoInverseSigTerm2 n A ) )  → ((P x1 ) → (P (invOL2 x1 ) ))) → (((x1 x2  : (OpPseudoInverseSigTerm2 n A ) )  → ((P x1 ) → ((P x2 ) → (P (opOL2 x1 x2 ) )))) → ((x  : (OpPseudoInverseSigTerm2 n A ))  → (P x )))))))
  ...
  stageOp : ((n  : Nat)  → ((OpPseudoInverseSigTerm n ) → (Staged (OpPseudoInverseSigTerm n ) )))
  ...
  stageOpE : ((n  : Nat ) (A  : Set )  → ((OpPseudoInverseSigTerm2 n A ) → (Staged (OpPseudoInverseSigTerm2 n A ) )))
