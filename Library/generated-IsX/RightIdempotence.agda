 ...
 module RightIdempotence  where
  ...
  record IsRightIdempotence (A  : Set ) (<|  : (A  → (A  → A )))  : Set where
    constructor IsRightIdempotenceC
    field
  ...
  record Hom (A1 A2  : Set ) (Ri1  : (RightIdempotence A1 )) (Ri2  : (RightIdempotence A2 ))  : Set where
    constructor HomC
    field
      hom : (A1 → A2)
      pres-<| : ({x1  : A1} {x2  : A1}  → (hom ((<| Ri1 ) x1 x2 ) ) ≡ ((<| Ri2 ) (hom x1 ) (hom x2 ) )) 
  
  record RelInterp (A1 A2  : Set ) (Ri1  : (RightIdempotence A1 )) (Ri2  : (RightIdempotence A2 ))  : Set₁ where
  ...
  simplifyB : (RightIdempotenceTerm  → RightIdempotenceTerm )
  ...
  simplifyOp : ((n  : Nat)  → ((OpRightIdempotenceTerm n ) → (OpRightIdempotenceTerm n )))
  simplifyOp _ (<|OL x1 x2 )  = (<|OL (simplifyOp _ x1 ) (simplifyOp _ x2 ) )
  
  simplifyOp _ (v x1 )  = (v x1 )
  
  simplifyOpE : ((n  : Nat ) (A  : Set )  → ((OpRightIdempotenceTerm2 n A ) → (OpRightIdempotenceTerm2 n A )))
  ...
  evalOp : ({A  : Set } (n  : Nat)  → ((RightIdempotence A ) → ((Vec A n ) → ((OpRightIdempotenceTerm n ) → A ))))
  evalOp n Ri vars (v x1 )  = (lookup vars x1 )
  
  evalOp n Ri vars (<|OL x1 x2 )  = ((<| Ri ) (evalOp n Ri vars x1 ) (evalOp n Ri vars x2 ) )
  
  evalOpE : ({A  : Set } (n  : Nat )  → ((RightIdempotence A ) → ((Vec A n ) → ((OpRightIdempotenceTerm2 n A ) → A ))))
  ...
  inductionOp : ((n  : Nat) (P  : ((OpRightIdempotenceTerm n ) → Set ))  → (((fin  : (Fin n ))  → (P (v fin ) )) → (((x1 x2  : (OpRightIdempotenceTerm n ) )  → ((P x1 ) → ((P x2 ) → (P (<|OL x1 x2 ) )))) → ((x  : (OpRightIdempotenceTerm n ))  → (P x )))))
  ...
  inductionOpE : ((n  : Nat ) (A  : Set ) (P  : ((OpRightIdempotenceTerm2 n A ) → Set ))  → (((fin  : (Fin n ))  → (P (v2 fin ) )) → (((x1  : A )  → (P (sing2 x1 ) )) → (((x1 x2  : (OpRightIdempotenceTerm2 n A ) )  → ((P x1 ) → ((P x2 ) → (P (<|OL2 x1 x2 ) )))) → ((x  : (OpRightIdempotenceTerm2 n A ))  → (P x ))))))
  ...
  stageOp : ((n  : Nat)  → ((OpRightIdempotenceTerm n ) → (Staged (OpRightIdempotenceTerm n ) )))
  ...
  stageOpE : ((n  : Nat ) (A  : Set )  → ((OpRightIdempotenceTerm2 n A ) → (Staged (OpRightIdempotenceTerm2 n A ) )))
