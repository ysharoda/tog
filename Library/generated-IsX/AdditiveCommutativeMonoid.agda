 ...
 module AdditiveCommutativeMonoid  where
  ...
  record IsAdditiveCommutativeMonoid (A  : Set ) (+  : (A  → (A  → A ))) (0ᵢ  : A )  : Set where
    constructor IsAdditiveCommutativeMonoidC
    field
      lunit_0ᵢ : ({x  : A }  → (+ 0ᵢ x ) ≡ x )
      runit_0ᵢ : ({x  : A }  → (+ x 0ᵢ ) ≡ x )
  ...
  record AdditiveCommutativeMonoid (A  : Set )  : Set where
    ...
    field
      + : (A  → (A  → A ))
      0ᵢ : A 
      isAdditiveCommutativeMonoid : (IsAdditiveCommutativeMonoid A (+) 0ᵢ ) 
