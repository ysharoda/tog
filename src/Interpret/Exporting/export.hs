...
module Interpret.Exporting.Export where 

import Tog.Raw.Abs
import Interpret.Utils.Lenses (name)
import Interpret.Utils.Bindings (getBindingArgs, getBindingExpr)
...
import Tog.Instrumentation.Conf (TargetLanguage(..)) 
import qualified Interpret.Exporting.Lean.Preprocess as LP
import qualified Interpret.Exporting.Agda.Preprocess as AP 
...
instance Export DeclOrLE where
  ...
  export _ (Lang_ _) = error "more modules need to be flattened."
