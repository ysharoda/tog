...
scopeCheckModule (C.Module _ _ (C.Lang_ defs)) =
  ...
  in scopeCheckModule $ processModule thrs -- $ appendToModule thrs $ map createHom $ readModuleRecords thrs
...
checkDecls ds ret = case ds of
  ...
  C.FunDef f _ _ : _ -> do
    ...
    clauses <- forM (zip clauses [1..]) $ \((ps, funDefBody), ix) -> do
      case funDefBody of
        C.FunDefBody body wheres0 -> do
          ...
          mapC checkPattern ps $ \ps -> do
            checkWheres f ix wheres $ \wheres -> do
              body <- checkExpr body
              return $ Clause ps $ ClauseBody body wheres
        C.FunDefNoBody -> do
          ps <- insertImplicitPatterns (srcLoc f) n ps
          mapC checkPattern ps $ \ps -> do
            return $ Clause ps ClauseNoBody
  ...
  where
    ...
    takeFunDefs _ [] =
      ([], [])
    takeFunDefs f (C.FunDef f' ps body : ds) | sameName f f' =
      first ((ps, body) :) $ takeFunDefs f ds
    takeFunDefs _ d =
      ([], d)
