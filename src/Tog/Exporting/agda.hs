...
-- from out experience, it is useful to have a boolean variable needed in
-- different places for different purposes. In some places it may not be
-- needed, so it can be ignored
class PrintAgda a where
  printAgda ::  a -> Doc

instance PrintAgda Name where
  printAgda nm = text $ replace (nm ^. name)
...
instance PrintAgda Constr where
  printAgda (Constr nm typ)  = printAgda nm <+> ofType <+> printAgda typ
...
instance PrintAgda Binding where
  printAgda binds =
    let arguments as = hsep $ map printAgda as
        binding x =  arguments (getBindingArgs x) <+> ofType <+> printAgda (getBindingExpr x)
    in case binds of
      Bind  _ _ -> parens $ binding binds
      HBind _ _ -> braces $ binding binds

instance PrintAgda [Binding] where
  printAgda bindsList = hsep $ map printAgda bindsList
...
instance PrintAgda TypeSig where
  printAgda (Sig nm e) =
    printAgda nm <+> ofType <+> printAgda e
...
instance PrintAgda Pattern where
  ...
  printAgda (ConP qnm ptrns) = parens $ printAgda qnm <+> (hsep $ map printAgda ptrns)
...
instance PrintAgda Import where
  ...
  printAgda (ImportArgs qn args) = printAgda qn <+> (hsep $ map printAgda args)

instance PrintAgda Decl where
  printAgda (TypeSig typ) = printAgda typ
  printAgda (FunDef nm patterns body) =
    printAgda nm <+> (hsep $ map (printAgda) patterns) <+> text fundef <+> printAgda body
...
recordHeader flds = ofType <+> universe <+> text record_beforeDecls
  where universe = maybe (text "Set") universeLevel flds
...
dataHeader ∷ Name -> Doc
dataHeader nm = ofType <+> printAgda nm <+> text type_beforeDecls

-- the config that needs to be imported --
ofType, lambdaArrow :: Doc
ofType = text ":"
lambdaArrow = text "→"

lambda, pi_representation, fun_sep, equality_symbol,
  record_keyword, record_beforeDecls, record_constructor, record_beforeFlds,
  type_keyword,type_beforeDecls,fundef, open, open_import,
  import_, module_, module_beforeDecls :: String
lambda = "λ"
pi_representation = "→"
fun_sep = "→"
