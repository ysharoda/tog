...
import           Tog.Prelude
import           Tog.PrettyPrint                  (Pretty, pretty, text)
...
-- backwards, so that @M.N.f@ will be stored as @'QName' f [N, M]@.
...
instance HasSrcLoc C.Empty where
  srcLoc (C.Empty ((l, c), _)) = SrcLoc l c
